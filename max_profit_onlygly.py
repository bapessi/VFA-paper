...
def integrate_ODE(p_control):
    ...
    return solver.t/24,B,GLY,t_list 


def get_total_GLY_fedbatch(t_eval,B,GLY):
    S0 = 1000
    Sin_gly = S0/v_M['GLY']
    alpha_4 = pr['k_4']*GLY/(GLY+pr['k_4']/pr['al_4']*(GLY/pr['KS_4']-1)**2)
    D = B*alpha_4/Sin_gly
...
def get_profit_fedbatch(p_control):
    ...
    GLY0 = 0.071*v_M['GLY']
    GLU0 = 0
    tf,B,GLY,t_eval = integrate_ODE(p_control) 
    GLYtotal = get_total_GLY_fedbatch(t_eval,B,GLY) 
    GLUtotal = 0
    Xf = B[-1]*v_M['X'] #g/L 
    return -(priceBio*Xf- GLYtotal*priceGly)/(tf*1e+6)
