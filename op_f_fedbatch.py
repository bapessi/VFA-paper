...
def min_time(params,y0,t_points,pr,fr):
    Ta = params[0]
    #t_vector = 24*np.array([Ta,Ta+0.25,Ta+0.5,Ta+0.75,Ta+1])
    t_vector = 24*np.array([Ta,Ta+0.1,Ta+0.2,Ta+0.3])
    D_array = params[1:]
    D_f = interpolate.interp1d(t_vector, D_array,kind='quadratic',fill_value=np.array([0]),bounds_error=False) 
    tf = min_stiff(y0,t_points,Ta,D_f,pr,fr)
    # print(D_array)
    print(tf/24)    
    return tf
    # Same as min_time but with fraction of glucose as p[1] 
def min_time_fraction(params,y0,t_points,pr):
    Ta = params[0]
    fr = params[1] 
    t_vector = 24*np.array([Ta,Ta+0.1,Ta+0.2,Ta+0.3])
    D_array = params[2:]
    print('darray ', D_array)
    print('fr ',fr)
    print('Ta ', Ta)
    D_f = interpolate.interp1d(t_vector, D_array,kind='quadratic',fill_value=np.array([0]),bounds_error=False) 
    tf = min_stiff(y0,t_points,Ta,D_f,pr,fr)
...
def min_stiff(y0,t_points,Ta,D_f,pr,fr):
    
      
    solver = ode(ODE_sys_ctrl_fed_batch)
    solver.set_integrator('lsoda')
    solver.set_f_params(Ta,D_f,pr,fr)
...
def ODE_sys_ctrl_fed_batch(t,y,Ta,D_f,pr,fr):
    ...
    if t < Ta*24:
        D = np.max((B*4.08788*alpha_4/(Sin_gly+GLY)
,B*2.07298*alpha_7/(Sin_glu+GLU))) 
    else:
        D = D_f(t)/Sin
