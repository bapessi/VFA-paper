...
def integrate_ODE_Xf(S0):
    ...
    while solver.successful() and solver.t <= tf:
        ...
        alpha_2 = pr['k_2']*res[variables.index('BUT')]/(res[variables.index('BUT')]+pr['k_2']/pr['beta_2']*(res[variables.index('BUT')]/pr['S_opt']-1)**2)*pr['k_d']/(res[variables.index('ACE')]+pr['k_d'])
    ...
    return solver.t,res,t_list,alpha_2_list
...
if __name__ == '__main__':
    ...
    S0 = (112,19)
    tf,res,t_list,alpha_2_list = integrate_ODE_Xf(S0)
    plt.plot(t_list,alpha_2_list)
    plt.show()
    print(tf)
    print(res)
