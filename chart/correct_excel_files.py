...
def get_feasible(df,metabolites_ignore):
    ...
    for m in df.index.values:
        if m =='5FTHF':
            pass
        elif not np.any(df.loc[m].values[df.loc[m].values>0]):
...
if __name__ == '__main__':
    ...
    file_input = 'data/' + filename + '_corrected.xlsx' # excel format for FBA and EFMs 
    file_output ='data/' + filename +'_feasible.xlsx' # final excel with feasible reactions and metabolites for FBA and EFMs 
    ...
    file_efm = 'data/main_biomass_forEFMs.xlsx'
    efms= ec.get_efms(file_efm)    
